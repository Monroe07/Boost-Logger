...
// Custom formatter factory to add Severity format support in config ini file.
...
class SeverityFormatterFactory :
    ...
    formatter_type create_formatter(const boost::log::attribute_name& name, const args_map& args)
    {
        ...
        if (it != args.end() || it == args.end()) {
            return boost::log::expressions::stream
                ...
                << boost::log::expressions::attr<boost::log::trivial::severity_level>(name);
        }
...
bool Logger::initFromConfig(const std::string& configFileName) {
    ...
    // Allows %Uptime(format=\"%O:%M:%S.%f\")% to be used in ini config file for property Format.
    boost::log::register_formatter_factory("Severity", boost::make_shared<SeverityFormatterFactory>());
...
bool Logger::disable() {
    ...
    if (!(boost::log::core::get()->get_logging_enabled())) {
        LOG_ERROR("Failed to disable Logging");
        return true;
    }
    else {
        return true;
...
bool Logger::addDataFileLog(const std::string& logFileName) {
